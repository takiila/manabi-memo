...
`ALLOWED_ACCOUNT_EMAILS` には利用する2〜3人だけをカンマ区切りで指定します。許可リスト外のFirebaseユーザーは同期APIとCampusへ403で拒否されます。各自が自分のアカウントをPC・スマートフォンの両方で使います。
...
5. 5 MB超と75 MB付近のPDFを同期し、Functionの4.5 MB上限を回避できることを確認する。
6. スマートフォンをオフラインにし、既存ノート・PDFの表示と編集保存を確認する。オンライン復帰後に明示同期する。
7. 同一アカウントでPC・スマートフォンを別々に編集し、revision競合を自動上書きせず選択画面で扱えることを確認する。
8. `main`へ更新をpushし、PC・スマートフォンに更新通知が出て新しい版へ切り替わることを確認する。
9. 発行から5分を過ぎた署名URLと、署名を外したR2 URLがPDFを返さないことを確認する。
