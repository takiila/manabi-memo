...
- Site: まなびメモ
- 公開版: v21相当
- 公開URL: 個人識別情報を含むため、公開リポジトリには記載しない
...
- 学期、時間割、講義、授業回
- 本文ノート、自動保存、表示設定
- PDF追加、端末保存、ページ表示、本文抽出、検索、任意の軽量参照・再追加
- 付箋、タグ、ピン、復習、振り返り、元位置
- ノート・PDF・付箋の検索、一覧・時間割表示
- バックアップ、復元、旧データ移行
- 共通ごみ箱、30日復元
- Campus Muster任意表示と既存データ構造
- 許可された2〜3人の共有カレンダー、共有メモ、課題の本人別完了、遊び候補の本人別参加可否、未回答者と科目別進捗
...
- 個人データと分離した共有カレンダーAPIを追加し、3アカウント間の予定共有、本人だけの完了更新、競合停止、30日ごみ箱・復元をHTTP統合テストで確認
- PDF本体・抽出情報を明示操作で削除し、ファイル名・総ページ数・最後のページ・メモ参照・SHA-256だけを残す軽量参照モードを追加
- 共有カレンダーへ遊び候補、本人別の参加可否、3色の登録者表示、学び/遊びフィルターを追加し、3アカウントのHTTP統合テストで全員の回答一致を確認
- デプロイ単位のService Worker生成と更新通知を実装し、「今すぐ更新」による新キャッシュへの切替を実ブラウザで確認
- コードフォルダをGit管理し、秘密情報・生成物を除外した初期コミットを作成

以上により、現行コードの主要機能、代表的なデータ移行、ローカル通常利用は確認しました。ただし、旧公開版との全機能を1対1で比較するfeature-by-feature監査と、外部サービス・物理端末を必要とする受入確認は完了扱いにしていません。
